...
def split_dataset(image_dir, label_dir, output_dir, val_size=0.2, test_size=0.1, random_state=42):
    ...
    if len(image_files) == 0 or len(label_files) == 0:
        raise ValueError("No images or labels found. Please check the dataset paths.")
...
if __name__ == "__main__":
    ...
    real_dirs = [
        ...
        # 추가 실제 데이터셋 폴더 경로를 여기에 추가
    ]

    # 가상 데이터 병합 및 준비
    synthetic_output_dir = os.path.join(script_dir, "yolo_synthetic_dataset")
    merge_datasets(synthetic_dirs, synthetic_output_dir, img_width, img_height, new_width, new_height)

    # 실제 데이터 병합 및 준비
    real_output_dir = os.path.join(script_dir, "yolo_real_dataset")
    merge_datasets(real_dirs, real_output_dir, img_width, img_height, new_width, new_height)

    # 데이터셋 분할 (가상 데이터셋)
    synthetic_split_output_dir = os.path.join(script_dir, "yolo_synthetic_split")
    train_dir, val_dir, test_dir = split_dataset(
        os.path.join(synthetic_output_dir, 'images'),
        os.path.join(synthetic_output_dir, 'labels'),
        synthetic_split_output_dir
    )
    
    # 학습을 위한 yaml 파일 생성 (가상 데이터셋)
    synthetic_data_yaml = """
    train: {}
    val: {}
    nc: 10
    ...
    with open(os.path.join(synthetic_split_output_dir, "data.yaml"), 'w') as f:
        f.write(synthetic_data_yaml)
    
    # 모델 예비 학습
    torch.cuda.empty_cache()
    os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'max_split_size_mb:128'
    if torch.cuda.is_available():
        print("Using GPU for pre-training")
    else:
        print("Using CPU for pre-training")
    
    train_yolo_model(os.path.join(synthetic_split_output_dir, "data.yaml"), epochs=50, batch_size=32, learning_rate=0.001, img_size=(new_width, new_height))

    # 모델 검증 (실제 데이터셋)
    real_split_output_dir = os.path.join(script_dir, "yolo_real_split")
    train_dir, val_dir, test_dir = split_dataset(
        os.path.join(real_output_dir, 'images'),
        os.path.join(real_output_dir, 'labels'),
        real_split_output_dir
    )

    # 학습을 위한 yaml 파일 생성 (실제 데이터셋)
    real_data_yaml = """
    train: {}
    val: {}
    nc: 10
    ...
    with open(os.path.join(real_split_output_dir, "data.yaml"), 'w') as f:
        f.write(real_data_yaml)

    if torch.cuda.is_available():
        print("Using GPU for validation and fine-tuning")
    else:
        print("Using CPU for validation and fine-tuning")
    
    # 모델 검증 및 개선
    results = validate_model(os.path.join(script_dir, "result_model/best.pt"), os.path.join(real_split_output_dir, "data.yaml"))
    print("Validation results on real data: ", results)

    # 필요에 따라 실제 데이터로 추가 학습 (fine-tuning)
    train_yolo_model(os.path.join(real_split_output_dir, "data.yaml"), epochs=50, batch_size=2, learning_rate=0.0005, img_size=(new_width, new_height))
    
    # 최종 모델 저장
    final_model_save_path = os.path.join(script_dir, "final_model", "best.pt")
